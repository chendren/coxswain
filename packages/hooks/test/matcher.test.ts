...
// ":" is the POSIX no-op builtin: it ignores its arguments and exits 0, so
// these commands both succeed and stay distinguishable by content in
// outcome.hook without depending on any real executable being on PATH.
describe("hook matcher selection", () => {
  ...
  it("R7.3: matcher is ignored on UserPromptSubmit", async () => {
    const cwd = await makeTmpCwd();
    await writeHooksJson(cwd, [
      { event: "UserPromptSubmit", matcher: "^bash$", command: ": prompt-hook" },
    ]);
    const engine = createHookEngine({ cwd, config: makeConfig(), env: testEnv() });
  ...
  it("R7.4: an invalid matcher regex skips the hook and records a continue outcome naming the pattern", async () => {
    const cwd = await makeTmpCwd();
    // Never spawned (skipped before runOne) — the command string doesn't
    // need to be a valid executable.
  ...
  it("R7.4: an invalid matcher does not prevent other valid hooks on the same event from running", async () => {
    const cwd = await makeTmpCwd();
    await writeHooksJson(cwd, [
      { event: "PreToolUse", matcher: "[unterminated", command: "broken-hook" }, // never spawned
